;(function(root, factory) {
	...
	var CitySelector = function(parentEl, options) {
		...
		this.actionName = this.options.actionName || { title: "请选择地区", cancelBtn: "取消", confirmBtn: "确认" }
		this.tabs = this.options.tabs || ["请选择", "请选择", "请选择"]
		this.data = this.options.defaultData || { province: "", city: "", district: "" }
		this.errorTips = this.options.errorTips || {
			noPro: "请选择省份",
			noCity: "请选择城市",
			noDis: "请选择区域"
		}
	...
	CitySelector.prototype.renderBody = function() {
	...
	CitySelector.prototype.getEl = function() {
		this.els = {
	...
	// 渲染城市选择
	CitySelector.prototype.renderCity = function() {
		...
		cityS = this._cityData[this.data.province] || {}
		...
		cityItems.forEach(function(item) {
			...
			item.addEventListener("click", function(e) {
				e.preventDefault()
				if (e.target.getAttribute("data-value") === _this.data.city) return
				cityItems.forEach(function(item) {
					if (item.classList.contains("active")) {
						item.classList.remove("active")
					}
				})
				_this.els.citySelTabs[1].innerHTML = _this.data.city = e.target.innerHTML
				item.classList.add("active")
				_this.els.citySelPro.style.display = "none"
				_this.els.citySelDis.style.display = "block"
				_this.renderDis()
				_this.data.district = ""
				_this.tabChange()
	...
	// 渲染区域选择
	CitySelector.prototype.renderDis = function() {
		var parentNode = this.parentEl.querySelector(".city-selector-district"),
			frag = document.createDocumentFragment(),
			aNode,
			districts,
			_this = this
		if (!this._cityData[this.data.province]) return
		districts = this._cityData[this.data.province][this.data.city] || []
		this._delAllNodes(parentNode)
		...
		disItems.forEach(function(item) {
			...
			item.addEventListener("click", function(e) {
				e.preventDefault()
				if (e.target.getAttribute("data-value") === _this.data.district) return
				disItems.forEach(function(item) {
					if (item.classList.contains("active")) {
						item.classList.remove("active")
					}
				})
				_this.els.citySelTabs[2].innerHTML = _this.data.district = e.target.innerHTML
				item.classList.add("active")
			})
		})
	}
	...
	// 改变class为active的元素位置
	CitySelector.prototype.activeAction = function() {
		var i, length
	...
	CitySelector.prototype.activeTab = function() {
	...
	// 显示错误信息
	CitySelector.prototype.showErrorModel = function() {
		var _this = this
		var show = function() {
			_this.els.errorTipModal.style.display = "block"
			setTimeout(function() {
				_this.els.errorTipModal.style.display = "none"
			}, 1500)
		}
	...
	// 绑定事件
	CitySelector.prototype.bindEvents = function() {
		var body = document.querySelector("body"),
			_this = this
		var fillData = function() {
			_this.fillData()
			if (_this.confirm) _this.confirm(_this.data)
		}
		this.els.citySelConfirmBtn.addEventListener("click", function() {
			if (_this.force === "true" && _this.showErrorModel()) {
				fillData()
			} else if (_this.force === "false") {
				fillData()
			}
		})
		...
		this.els.citySelInput.addEventListener("click", function() {
			if (_this.els.citySelBody.style.display === "block") {
				_this.els.citySelBody.style.display = "none"
			} else {
				_this.els.citySelBody.style.display = "block"
			}
			_this.activeAction()
		})
